...
#define BFAST_VERSION = { 1, 0, 1, "2019.9.24" };
    ...
    struct alignas(8) Header {
        ulong magic = 0ul;         // Either MAGIC (same-endian) of SWAPPED_MAGIC (different-endian)
        ulong data_start = 0ul;    // >= desc_end and modulo 64 == 0 and <= file_size
        ulong data_end = 0ul;      // >= data_start and <= file_size
        ulong num_arrays = 0ul;    // number of array_headers
    };

    // A helper struct for representing a range of bytes 
    struct ByteRange {
        const byte* _begin = nullptr;
        const byte* _end = nullptr;
